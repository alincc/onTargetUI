...
function getNextVersionName(req, res) {
  ...
  if(fs.existsSync(fullFilePath)) {
    ...
    res.send({
      success: true,
      newVersionName: versionName + '/' + fileNameWithoutExt + '_v' + (parseInt(totalVersions) + 1) + '.' + fileExt
    });
  } else {
    res.status(400);
...
function _getPages(relativePath) {
  ...
  var files = _.filter(fs.readdirSync(pageFolder), function(fn) {
    return fs.lstatSync(path.join(pageFolder, fn)).isFile();
  });
  return _.map(files, function(el) {
    return makePath(el);
  });
}
