...
THREE.AssimpJSONLoader.prototype = {
    ...
    load: function ( url, rootObject, onLoad, onProgress, onError ) {
	...
	loader.load( url, function ( text ) {
	    ...
	    //metadata check pulled out for now because we dont prepend the metadata tag to the split files. 
	    
	    //no more return, function now mutates rootnode instead so loaders can be chained without recursion
	    scope.parse( json , rootObject);
	    onLoad( scene );
    ...
    parse: function ( json, rootobj ) {
	if(json.meshes){
	    var meshes = this.parseList ( json.meshes, this.parseMesh );
	    
	    for (var i = 0; i < meshes.length; i++){
		rootobj.meshes.push(meshes[i]);
		rootobj.jsonMeshes.push(json.meshes[i]);
	    }
	...
	if(json.materials){
		...
		rootobj.materials.push(materials[i]);		
	    }
	}
	if(json.nodes){
	    for (var i = 0; i < json.nodes.length; i++){
		rootobj.flatrefs.push(this.parseObject( json, json.nodes[i], meshes, materials ));
	    }
	}
	...
	obj.meshes = node.meshes || "";
	obj.mparent = node.parent;
	//better have a parent or something is broken 


	//shouldn't be any children but we will leave this in for now...
